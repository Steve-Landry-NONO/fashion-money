...
def record_action(session: Session, user_id: str, option: Option, action: str) -> tuple[Decision, DecisionAction]:
    if action not in ALLOWED_ACTIONS:
        raise ValueError("invalid action")
    ev = evaluate(session, user_id, option)
    decision = Decision(
        user_id=user_id, look_id=None, option_id=option.id, verdict=ev.verdict,
        available_at=ev.available, price=ev.price,
    )
    ...
    session.add(row)
    session.commit()
    emitter.emit(emitter.DECISION_ACTION_TAKEN, user_id, action=action, verdict=ev.verdict)
    if action == "phase":
...
def confirm_purchase(
    ...
    *, fail_after_spend_for_test: bool = False,
    ...
    ledger = BudgetLedger(
        user_id=user_id, period=current_period(), type="SPEND", amount=float(option.price),
        ref_purchase_id=purchase.id, idempotency_key=f"purchase:{idempotency_key}",
    )
    session.add(ledger)
    session.flush()
    ...
    item = WardrobeItem(
        user_id=user_id, category=piece.category, color=piece.color, cut=piece.cut,
        material=piece.material, price=float(option.price), source="PURCHASE",
    )
    session.add(item)
    session.flush()
    purchase.wardrobe_item_id = item.id
    session.commit()
    emitter.emit(emitter.PURCHASE_CONFIRMED, user_id, price=float(option.price), pieces_added=1)
    emitter.emit(emitter.WARDROBE_ITEM_ADDED, user_id, source="PURCHASE", category=piece.category)
    return purchase
